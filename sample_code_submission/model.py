...
class Model:
    def __init__(self, get_train_set=None, systematics=None, model_type="sample_model", pretrained_model_path=None):
        import os
        self.pretrained_model_path = pretrained_model_path or os.environ.get("PRETRAINED_MODEL_PATH", None)
        ...
        if model_type == "BDT":
            ...
            if self.pretrained_model_path and os.path.exists(os.path.join(self.pretrained_model_path, "keras_model.h5")):

                self.model = NeuralNetwork.load(self.pretrained_model_path)
                self.skip_training = True
                print(f"Réseau NN chargé depuis {self.pretrained_model_path}")
            else:
                self.model = NeuralNetwork(train_data=self.training_set["data"])
                self.skip_training = False
                print(" Nouveau réseau NN initialisé")
    ...
    def fit(self):
        if getattr(self, "skip_training", False):
            print("Entraînement sauté (modèle pré-entraîné)")
            return
        ...
        for key in holdout_results:
            print(f"\t{key} : {holdout_results[key]}")
        ...
        histogram_dataset(
            self.valid_set["data"], self.valid_set["labels"], self.valid_set["weights"], columns=["score"]
        )

        stacked_histogram(
            self.valid_set["data"], self.valid_set["labels"], self.valid_set["weights"],
            self.valid_set["detailed_labels"], "score"
        )
        ...
        save_path = self.pretrained_model_path or os.path.join(os.path.dirname(__file__), "saved_nn")
