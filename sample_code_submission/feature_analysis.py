...
def feature_correlations(data_set):
    ...
    label_col = next((c for c in data_set.columns if c.lower() in ["label", "labels"]), None)
    ...
    for jet_cat in ["0j", "1j", "2j+"]:
        for label_val in [1, 0]:
            df_filtered = data_set[(data_set["jet_category"] == jet_cat) & (data_set[label_col] == label_val)]
            ...
            drop_cols = [label_col, 'weights'] if 'weights' in df_numeric.columns else [label_col]
            ...
            df_numeric = df_numeric.dropna(axis=1, thresh=int(0.9 * len(df_numeric)))  # keep cols with >90% non-NaN
 
            if df_numeric.shape[1] < 2:
                print(f"⚠️ Too few valid numeric columns in {jet_cat} - {captions[label_val]}")
                continue
            ...
            plt.title(f"{captions[label_val]} - {jet_cat} - Full Feature Correlation", fontsize=14)
            plt.xticks(rotation=45, ha="right")
            plt.yticks(rotation=0)
            plt.tight_layout()
            plt.show()

features_all = ['PRI_lep_pt', 'PRI_lep_eta', 'PRI_lep_phi', 'PRI_had_pt', 'PRI_had_eta', 'PRI_had_phi', 'PRI_jet_leading_pt', 'PRI_jet_leading_eta', 'PRI_jet_leading_phi', 'PRI_jet_subleading_pt', 'PRI_jet_subleading_eta', 'PRI_jet_subleading_phi', 'PRI_n_jets', 'PRI_jet_all_pt', 'PRI_met', 'PRI_met_phi','DER_mass_transverse_met_lep', 'DER_mass_vis', 'DER_pt_h', 'DER_deltaeta_jet_jet', 'DER_mass_jet_jet', 'DER_prodeta_jet_jet', 'DER_deltar_had_lep', 'DER_pt_tot', 'DER_sum_pt', 'DER_pt_ratio_lep_had', 'DER_met_phi_centrality', 'DER_lep_eta_centrality']
    ...
    for bias_name, bias_info in biases.items():
        for shift in ["+1σ", "-1σ"]:
            ...
            kwargs = {
                ...
                "ttbar_scale": 1.0, "diboson_scale": 1.0, "bkg_scale": 1.00
            }
            ...
            for feat in features:
                ...
                chi2_results[f"{feat} (background)"][f"{bias_name} ({shift})"] = chi2_bkg
